...
export class ListaCircularSimple{
    ...
    dibujarNodos(valor) {
        ...
        while (tmp != null) {
            ...
            if (valor != undefined && tmp.info == valor) {
                //Dibujar rectangulo
                ctx.beginPath();
                ctx.fillStyle = "rgb(71,125,6)"; //VERDE OSCURO
                ...
                //Dibujar rectangulo
                ctx.beginPath();
                ctx.fillStyle = "rgb(175,224,58)"; //VERDE CLARO
                ctx.fillRect(x, y, 55, 30);
            }
    ...
    dibujarCola() {
        ...
        if (contador != 0) {
            ...
            var x = ((contador - 1) * 80);
            var y = 0;
            var aux = x + 30 - ((contador - 1) * 80); //x + 30 - algo
            ctx.beginPath();
            ctx.fillStyle = "black";
            ctx.moveTo(x + 80, y + 15); //empieza linea desde el final
            ctx.lineTo(x + 80, y + 60);
            ctx.moveTo(x + 80, y + 60); //linea final hasta el nodo
            ctx.lineTo(aux, y + 60);
            ctx.moveTo(aux, y + 60); //conectar linea al nodo
            ctx.lineTo(aux, y + 35);
            ctx.closePath();
            ctx.stroke();
            //cabeza de la flecha
            ctx.beginPath();
            ctx.fillStyle = "black";
            ctx.moveTo(aux - 5, y + 35);
            ctx.lineTo(aux, y + 30);
            ctx.lineTo(aux + 5, y + 35);
            ctx.closePath();
            ctx.fill();
        }
    }
